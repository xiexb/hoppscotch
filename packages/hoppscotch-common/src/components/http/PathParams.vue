...
const emit = defineEmits<{
  (e: "update:modelValue", value: Array<HoppRESTPathParam>): void
}>()
...
// Ensure params is never undefined (can happen with v17 persisted tab data)
if (!params.value) {
  params.value = []
}
...
watch(
  ...
  (newParamsList) => {
    ...
    const filteredWorkingParams: HoppRESTPathParam[] = pipe(
      workingParams.value,
      A.filterMap(
        flow(
